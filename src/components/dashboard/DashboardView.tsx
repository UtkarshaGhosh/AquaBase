...
export const DashboardView = () => {
    ...
    const speciesForFilter = React.useMemo(() => {
        if (uploadedRecords && uploadedRecords.length > 0) {
            ...
            uploadedRecords.forEach((r: any) => {
                ...
                if (!set.has(common))
                    set.set(common, { id: common, common_name: common, scientific_name: scientific });
            });
            return Array.from(set.values());
        }
        return species;
    }, [uploadedRecords, species]);
    const [locationOptions, setLocationOptions] = React.useState<Array<{
        id: string;
        label: string;
        lat?: number;
        lon?: number;
        ...
        return (combinedData || []).filter((r: any) => {
            if (filters.species) {
                ...
                if (entry) {
                    ...
                    if (!(common === entry.common_name || sci === entry.scientific_name || r.species?.id === sel))
                        return false;
                }
                else {
                    if (r.species?.id && r.species.id !== sel)
                        return false;
                }
            }
            if (filters.dateFrom) {
                const d = r.catch_date ? new Date(r.catch_date) : null;
                if (!d || d < filters.dateFrom)
                    return false;
            }
            if (filters.dateTo) {
                const d = r.catch_date ? new Date(r.catch_date) : null;
                if (!d || d > filters.dateTo)
                    return false;
            }
            if (filters.location) {
                if (r.latitude === undefined || r.longitude === undefined)
    ...
    if (error) {
        return (<Alert variant="destructive" className="m-6">
    ...
    return (<div className="min-h-screen bg-gradient-surface">
      <div className="container mx-auto p-6 space-y-6">
        ...
        <StatsCards totalCatches={stats.totalCatches} dateRange={stats.dateRange} totalWeight={stats.totalWeight} avgWeightPerCatch={stats.avgWeightPerCatch} topSpecies={stats.topSpecies}/>

        <div className="space-y-6">
          
          <div>
            <FilterPanel filters={tempFilters} onFiltersChange={setTempFilters} onExport={handleExport} onFindHotspots={handleFindHotspots} species={speciesForFilter} locations={locationOptions} isLoading={isLoading || exporting} onApply={() => setFilters(tempFilters)}/>
          </div>
            ...
            <CardContent>
              <div className="overflow-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredData.slice(0, 20).map((r: any, i: number) => (<tr key={r.id || i} className="border-t">
                    ...
                    {filteredData.length === 0 && (<tr><td colSpan={6} className="p-2 text-muted-foreground">No records found.</td></tr>)}
                  </tbody>
                </table>
              </div>
            </CardContent>
